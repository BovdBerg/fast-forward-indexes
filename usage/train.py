...
class AvgWeightsClassifier(nn.Module):
    ...
    #     Args:
    #         d_reps (np.ndarray): List of top-ranked document embeddings for a given query.

    #     Returns:
    #         weights (Sequence[float]): Learned weights for each document embedding based on position/scores.
    #     """
    #     # TODO: rewrite method to fit my needs
    #     uniform = np.ones(len(d_reps)) / len(d_reps)
    #     return uniform
